...
public class WattBattery<T extends DirectionalWattProvidable> implements WattStorable {
    ...
    /**
     * Create a new {@link com.quattage.mechano.foundation.electricity.watt.WattBatteryBuilder <code>WattBatteryBuilder</code>} bound to the given Object.
     * <p>
     * <pre>
    ...
    public boolean canExtract() {
        if(maxFlux.getRaw() < -32767) return false;
        if(storedWattTicks > 0) return true;
        else storedWattTicks = 0;
        return false;
    }

    @Override
    public boolean canReceive() {
        if(maxTolerance.getRaw() < -32767) return false;
        if(storedWattTicks <= maxStoredWattTicks) return true;
        else storedWattTicks = maxStoredWattTicks;
        return false;
    }
    ...
    public WattUnit extractWatts(final WattUnit maxWattsToExtract, boolean simulate) {
        ...
        float roundedDiff = getCloseEnoughToEmpty();
        wattsExtracted += roundedDiff;
    ...
    public float getCloseEnoughToFull() {
        float out = Math.abs((float)maxStoredWattTicks - storedWattTicks);
        if(out < 0.25f) {
            storedWattTicks = maxStoredWattTicks;
            return out;
        }
    ...
    public float getCloseEnoughToEmpty() {
        float out = storedWattTicks;
        if(out < 0.25f) {
            storedWattTicks = 0;
            return out;
        }
